...
insert into products (name,price,coffee_origin) values ('Latte',3.5,'Indonesia'),('Americano',3.00,'Brazil'), ('Flat White',3.5,'Indonesia'), ('Filter',3.0,'India');

select * from products;
update products set coffee_origin = 'Sri Lanka' where id = 7;
-- Safe Updates = Id Only --
...
select * from products where price = 3 or coffee_origin = 'Colombia';
use coffee_store;
...
select count(first_name) from customers where gender = 'M' and phone_number IS NULL;

select * from customers where last_name in ('Taylor','Bluth','Armstrong');
select * from customers where first_name Not IN ('Katie','George','John');
...
select distinct customer_id from orders where order_time between '2017-02-01' and '2017-02-28' order by customer_id;
